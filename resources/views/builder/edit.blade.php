        ...
        <div class="relative z-10 landing-container max-w-3xl">
            ...
            <div class="mb-8">
                ...
                <div class="mt-4 flex flex-wrap items-center gap-3">
                    <span class="rounded-full px-3 py-1 text-xs font-semibold {{ $invitation->isPublished() ? 'bg-luxury-emerald/15 text-luxury-emerald' : 'bg-ink/10 text-ink-muted' }}">
                        {{ $invitation->isPublished() ? 'Nashr qilingan' : 'Qoralama' }}
                    </span>
                    @if ($invitation->isPublished())
                        <a href="{{ route('invitation.show', $invitation->slug) }}" target="_blank" class="text-sm font-semibold text-luxury-gold-dark hover:underline">
                            Ko'rish → /i/{{ $invitation->slug }}
                        </a>
                    @endif
                </div>
            </div>
            ...
            <div class="mb-8 grid grid-cols-3 gap-3">
                <div class="glass-luxury rounded-xl p-4 text-center">
                    <p class="font-serif text-2xl font-bold text-ink">{{ $stats['attending'] }}</p>
                    <p class="text-xs text-ink-muted mt-1">Keladi</p>
                </div>
                <div class="glass-luxury rounded-xl p-4 text-center">
                    <p class="font-serif text-2xl font-bold text-ink">{{ $stats['declined'] }}</p>
                    <p class="text-xs text-ink-muted mt-1">Kelmaydi</p>
                </div>
                <div class="glass-luxury rounded-xl p-4 text-center">
                    <p class="font-serif text-2xl font-bold text-ink">{{ $stats['total_guests'] }}</p>
                    <p class="text-xs text-ink-muted mt-1">Jami mehmon</p>
                </div>
            </div>
