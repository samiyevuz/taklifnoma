<header class="nav-glass" id="site-nav" role="banner">
    <div class="nav-glass__inner mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center gap-2 sm:gap-3">
            ...
            @else
                <a href="{{ route('login') }}" class="hidden text-sm font-semibold text-ink-soft hover:text-ink sm:inline-flex">{{ __('nav.login') }}</a>
                <a href="{{ route('register') }}" class="hidden btn-outline-luxury text-sm sm:inline-flex">{{ __('nav.register') }}</a>
            @endauth
            ...
            <button type="button" id="mobile-menu-toggle" class="flex h-10 w-10 items-center justify-center rounded-xl glass-luxury lg:hidden" aria-label="{{ __('nav.open_menu') }}" aria-expanded="false" aria-controls="mobile-nav">
    ...
    <nav class="mobile-nav__panel" aria-label="{{ __('nav.mobile_nav') }}">
        ...
        <ul class="flex flex-col gap-1">
            ...
            @auth
                ...
                <li><a href="{{ route('account.orders') }}" class="block rounded-lg px-4 py-3 text-base font-medium text-ink hover:bg-white/50" data-close-mobile-nav>{{ __('nav.my_orders') }}</a></li>
                <li><a href="{{ route('account.favorites') }}" class="block rounded-lg px-4 py-3 text-base font-medium text-ink hover:bg-white/50" data-close-mobile-nav>{{ __('nav.favorites') }}</a></li>
            @else
                <li><a href="{{ route('login') }}" class="block rounded-lg px-4 py-3 text-base font-medium text-ink hover:bg-white/50" data-close-mobile-nav>{{ __('nav.login') }}</a></li>
                <li><a href="{{ route('register') }}" class="block rounded-lg px-4 py-3 text-base font-medium text-ink hover:bg-white/50" data-close-mobile-nav>{{ __('nav.register') }}</a></li>
            @endauth
        </ul>
        <div class="mt-6 flex flex-col gap-3 border-t border-white/40 pt-6">
            <button type="button" id="theme-toggle-mobile" class="theme-toggle-btn rounded-full px-4 py-3 text-sm font-medium glass-luxury text-ink-soft" data-theme-label-dark="{{ __('nav.theme_dark') }}" data-theme-label-light="{{ __('nav.theme_light') }}">
                {{ __('nav.theme_dark') }}
            </button>
