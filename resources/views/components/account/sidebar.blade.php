...
<aside class="account-sidebar glass-luxury rounded-2xl p-4">
    <div class="account-user-card">
        <div class="account-avatar" aria-hidden="true">{{ $user->initials() }}</div>
        <div class="min-w-0">
    ...
    <nav class="account-nav" aria-label="Kabinet navigatsiyasi">
        <a href="{{ route('account.dashboard') }}" class="account-nav__link {{ str_starts_with($current, 'account.dashboard') ? 'is-active' : '' }}">
        ...
        <a href="{{ route('account.orders') }}" class="account-nav__link {{ str_starts_with($current, 'account.orders') ? 'is-active' : '' }}">
        ...
        <a href="{{ route('account.favorites') }}" class="account-nav__link {{ str_starts_with($current, 'account.favorites') ? 'is-active' : '' }}">
    ...
    <div class="mt-6 pt-4 border-t border-white/40">
        <a href="{{ route('builder.create') }}" class="btn-gold-shimmer btn-shine w-full text-center text-sm" data-ripple>
            + Yangi taklifnoma
        </a>
        <form action="{{ route('logout') }}" method="POST" class="mt-3">
            @csrf
            <button type="submit" class="btn-outline-luxury w-full text-sm">Chiqish</button>
        </form>
    </div>
</aside>
